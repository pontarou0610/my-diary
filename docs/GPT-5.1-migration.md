...
2. `OPENAI_PREFERRED_MODELS`（優先モデル配列。JSON配列またはカンマ区切り）
3. スクリプト内の既定優先リスト（`gpt-5.1` → `gpt-5.2` → `gpt-5` → `gpt-4.1` → `gpt-4o-mini`）
...
## 代替手順（設定ファイルで固定）
`config/ai-model.json` の `openai.defaultModel` を `gpt-5.1` に変更します。
